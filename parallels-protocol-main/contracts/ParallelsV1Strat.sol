...
contract YTokenStrat is IStrat {
    ...
    string public name = "Parallels V2"; // for display purposes only
    ...
    function divest(uint amount) external override onlyVault {
        ...
        if(balance < amount) {
            ...
            require(missingAmount <= withdrawalCap, "Reached withdrawal cap"); // Big withdrawals can cause slippage on Parallels's side. Users must split into multiple txs
            yToken.withdraw(
                Math.min(
    ...
    function totalParallelsDeposits() public view returns (uint) {
    ...
    function calcTotalValue() external view override returns (uint) {
        return Math.max(totalParallelsDeposits(), 1) // cannot be lower than 1 because we subtract 1 after
    ...
    // Bypasses withdrawal cap. Should be used with care. Can cause Parallels slippage with large amounts.
    ...
    // set buffer to -1 to pause deposits to Parallels. 0 to remove buffer.
